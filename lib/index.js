...
/**
 * Legacy in-workspace trash directory name.
 *
 * The store used to live here, which meant a plugin whose entire purpose is
 * keeping a workspace clean left a directory of its own in that workspace --
 * and made every quarantine a same-volume rename. It now lives under the user
 * profile instead (see storeRootFor). The name survives for two reasons: it
 * stays guarded against the sweep, so a stray copy left by an older version is
 * never collected as if it were user scratch, and it is the marker that
 * identifies a legacy workspace-relative `storedAt`.
 */
const TRASH_DIR = ".dsh-scratch-trash";

/** Default store-root parent, relative to the user's home directory. */
const STORE_PARENT = join(".dsh", "turn-scratch");
...
const DEFAULTS = {
	...
	__faults: [],
	/**
	 * 存储根 (store root):隔离桶与暂存区放哪儿。
	 *
	 * 空串(默认)= `~/.dsh/turn-scratch/<工作区指纹>/`,即放在用户目录、工作区之外 ——
	 * 这个插件存在的意义就是让工作区保持干净,它自己的存储没道理待在里面。
	 * 绝对路径 = 用指定位置。
	 * 字面量 "workspace" = 回到旧行为(工作区内的 .dsh-scratch-trash),留作回退。
	 *
	 * ⚠️ 跨卷代价:工作区与存储根不在同一个卷时,rename 会 EXDEV 失败,搬运退化为
	 * 「复制 → 校验 → 落库 → 删源」。那是本插件在人工 purge 之外唯一会删除原始文件的
	 * 地方,已按「任何时刻崩溃都保留原件」的顺序实现,但会真的拷字节(大目录会慢)。
	 * 想避开就把它指向与工作区同卷的路径。
	 */
	storeRoot: "",
	/**
	 ...
	 *
	 * 相对路径按存储根解析(默认 `work`,即 `<存储根>/work`),因而天然在工作区之外,
	 * 不会被普通候选路径(标记/启发式/目录mtime)收第二遍 —— 只由归档步骤处理。
	 * 绝对路径按字面量用,想放回工作区内就写绝对路径。
	 * 设为空串可关闭该功能。
	 */
	workDir: "work",
...
/**
 * Stable on-disk key for one workspace's store.
 *
 * A readable basename plus a short hash of the normalized absolute path, so two
 * workspaces that happen to share a basename never share a store, while the
 * directory stays identifiable when a human browses it.
 * @param absoluteRoot - absolute workspace root.
 * @returns the directory name.
 */
function workspaceKey(absoluteRoot) {
	const absolute = resolve(absoluteRoot);
	// Case-fold the WHOLE key, slug included. Windows paths are case-insensitive,
	// so D:\Share and d:\share are one workspace; hashing the folded path but
	// keeping the original case in the slug would hand that one workspace two
	// differently-named stores depending on how the harness happened to spell the
	// cwd. Folding everything makes the key a pure function of the folded path.
	const folded = toPosix(absolute).toLowerCase();
	const hash = createHash("sha1").update(folded).digest("hex").slice(0, 8);
	const slug = basename(absolute).toLowerCase().replace(/[^\w.-]+/g, "_").replace(/^_+|_+$/g, "").slice(0, 32);
	return (slug.length > 0 ? slug : "workspace") + "-" + hash;
}

/**
 * Resolve the store root holding one workspace's quarantine buckets.
 *
 * Defaults to `<home>/.dsh/turn-scratch/<workspaceKey>`, deliberately OUTSIDE
 * the workspace: this plugin exists to keep a workspace clean, so its own
 * storage has no business sitting in there. A configured `storeRoot` overrides
 * the location, and the literal "workspace" restores the old in-workspace
 * behaviour if that ever needs to come back.
 * @param absoluteRoot - absolute workspace root.
 * @param configured - the `storeRoot` option.
 * @returns the absolute store root.
 */
function storeRootFor(absoluteRoot, configured) {
	if (typeof configured === "string") {
		const trimmed = configured.trim();
		if (trimmed === "workspace") return join(resolve(absoluteRoot), TRASH_DIR);
		if (trimmed.length > 0) return resolve(trimmed);
	}
	return join(homedir(), STORE_PARENT, workspaceKey(absoluteRoot));
}

/**
 * Recursively measure a file or directory as {files, bytes}, never following
 * symlinks. This exists to verify a cross-volume copy rather than trust it.
 * @param target - absolute path.
 * @returns the file count and total bytes.
 */
	...
	const walk = async (dir) => {
		for (const entry of await readdir(dir, { withFileTypes: true })) {
			...
			try {
				...
				// A file that vanished mid-walk cannot be measured. Leave it at
				// zero: the mismatch below is the signal that matters.
			}
		}
	};
	await walk(target);
...
/**
 * Move one file or directory, atomically when the platform allows it.
 *
 * Same volume: rename() is atomic, instant, and copies nothing -- take it.
 * Cross volume (EXDEV) there is no choice but to copy the bytes, and that copy
 * is the only place in this plugin -- outside the human-invoked purge -- that
 * ever removes an original. The ORDER is the entire safety argument:
 *
 *   1. copy source -> target.part     (a crash here leaves the original alone)
 *   2. verify file count and bytes    (a short copy is caught, not trusted)
 *   3. rename target.part -> target   (durable in the store only now)
 *   4. remove the source              (only now, and only the verified one)
 *
 * A crash at any step leaves the original intact, so the worst case is a
 * harmless duplicate rather than a hole. `preserveTimestamps` keeps the mtimes
 * the orphan-store and scratch-dir age heuristics read.
 * @param source - absolute path to move.
 * @param target - absolute destination.
 * @param forceCopy - TEST-ONLY: skip the rename fast path so the copy branch can
 *   be exercised on a single-volume machine. Driven by `__faults`; never set it
 *   in a real profile.
 * @returns "rename" or "copy", for logging and tests.
 */
async function relocate(source, target, forceCopy) {
	...
	await cp(source, staging, { recursive: true, preserveTimestamps: true, dereference: false, force: true });
	const [from, to] = await Promise.all([measure(source), measure(staging)]);
	if (from.files !== to.files || from.bytes !== to.bytes) {
		await rm(staging, { recursive: true, force: true });
		throw new Error(
			"cross-volume copy came out incomplete (" + to.files + " files/" + to.bytes + " bytes copied vs " +
			from.files + " files/" + from.bytes + " bytes expected); original left untouched"
		);
	}
...
/**
 * Absolute path of the file backing one manifest item.
 *
 * `storedAt` is store-relative. Manifests written before the store moved out of
 * the workspace carry a workspace-relative path that still begins with the
 * legacy trash prefix; recognise that shape and resolve it against the
 * workspace, so an un-migrated manifest keeps restoring instead of quietly
 * reporting its files as missing.
 * @param storeRoot - absolute store root.
 * @param workspaceRoot - absolute workspace root.
 * @param item - the manifest record.
 * @returns the absolute stored path.
 */
function storedSource(storeRoot, workspaceRoot, item) {
	const stored = toPosix(item.storedAt);
	if (stored === TRASH_DIR || stored.startsWith(TRASH_DIR + "/")) return join(workspaceRoot, stored);
	return join(storeRoot, stored);
}

/**
 * Absolute path an item should be restored to.
 *
 * Items carry an `origin`. "workspace" -- the default, and what every legacy
 * item implicitly is -- restores under the workspace at its recorded relative
 * path. "store" covers the staging area, which now lives inside the store root
 * and so is addressed relative to it.
 * @param workspaceRoot - absolute workspace root.
 * @param storeRoot - absolute store root.
 * @param item - the manifest record.
 * @returns the absolute restore destination.
 */
function restoreTarget(workspaceRoot, storeRoot, item) {
	const rel = toPosix(item.path);
	return item.origin === "store" ? join(storeRoot, rel) : join(workspaceRoot, rel);
}
...
/**
 * Move one path into the store, preserving its position under `base`.
 *
 * `storedAt` is always store-relative, which is what lets one bucket hold both
 * workspace scratch (addressed against the workspace) and staging leftovers
 * (addressed against the store) without the two ever being confused.
 * @param storeRoot - absolute store root; the base for `storedAt`.
 * @param base - the root `source` is addressed against.
 * @param source - absolute path being quarantined.
 * @param bucket - absolute bucket directory.
 * @param kind - manifest reason tag.
 * @param origin - "workspace", or "store" for the staging area.
 * @returns the manifest record.
 */
async function quarantine(storeRoot, base, source, bucket, kind, origin, forceCopy) {
	const rel = toPosix(relative(base, source));
	...
	const mode = await relocate(source, target, forceCopy);
	const item = { path: rel, kind, storedAt: toPosix(relative(storeRoot, target)) };
	if (origin === "store") item.origin = "store";
	// Only recorded when it happened: absent means the cheap atomic rename.
	if (mode === "copy") item.moved = "copy";
	return item;
}

/**
 * Move one quarantined item back to where it came from.
 *
 ...
 */
async function restoreItem(workspaceRoot, storeRoot, item, by, forceCopy) {
	const source = storedSource(storeRoot, workspaceRoot, item);
	const target = restoreTarget(workspaceRoot, storeRoot, item);
	if (!existsSync(source)) return false;
	if (existsSync(target)) return false;
	await relocate(source, target, forceCopy);
...
/**
 * Enumerate quarantine buckets under a store root.
 * @param storeRoot - absolute store root.
 * @param stagingName - name of the staging directory to skip, if any.
 * @returns bucket descriptors, each with its parsed manifest when readable.
 */
async function listBuckets(storeRoot, stagingName) {
	if (!existsSync(storeRoot)) return [];
	const buckets = [];
	let sessions;
	try {
		sessions = await readdir(storeRoot, { withFileTypes: true });
	} catch {
		return [];
	}
	for (const sessionEntry of sessions) {
		if (!sessionEntry.isDirectory()) continue;
		// The staging area is a sibling of the buckets, not one of them.
		if (stagingName && sessionEntry.name === stagingName) continue;
		const sessionDir = join(storeRoot, sessionEntry.name);
		let turns;
		try {
...
export function apply(ctx, rawConfig) {
	...
	/**
	 * Raw staging spec, or null when disabled. A relative spec resolves against
	 * the STORE root, not the workspace: with the default store the staging area
	 * therefore sits outside the workspace, which is the point. An absolute spec
	 * is used as given, which is how you put it back inside a workspace.
	 */
	const workSpec = typeof options.workDir === "string" && options.workDir.trim().length > 0 ? options.workDir.trim() : null;
	const staticProtected = [...ALWAYS_PROTECTED, ...options.protect].map(globToRegExp);
	const matchesScratch = (rel) => compiled.some((re) => re.test(rel));
	/**
	 * TEST-ONLY: force the cross-volume copy branch of `relocate` so it can be
	 * exercised on a machine where the workspace and the store share a volume.
	 * Without this the branch that actually deletes originals would never run in
	 * CI, which is exactly the wrong thing to leave untested.
	 */
	const forceCopy = Array.isArray(options.__faults) && options.__faults.includes("cross-volume");

	/** Absolute staging directory for one store root, or null when disabled. */
	const workDirFor = (storeRoot) => {
		if (workSpec === null) return null;
		return isAbsolute(workSpec) ? resolve(workSpec) : resolve(storeRoot, workSpec);
	};

	/**
	 * Build the protection predicate for one sweep.
	 *
	 * Only needed when the staging area lands INSIDE the workspace — the archive
	 * step owns it there, and without the guard a busy staging dir would be
	 * collected twice or trip the scratch-directory scan. With the staging area
	 * outside the workspace (the default) the scan cannot reach it anyway, so the
	 * guard is simply absent.
	 * @param workRel - workspace-relative staging path, or null when it is outside.
	 * @returns the predicate.
	 */
	const protectorFor = (workRel) => {
		const guards = workRel === null
			? []
			: [globToRegExp(workRel.replace(/\/+$/, "") + "/**"), globToRegExp(workRel.replace(/\/+$/, ""))];
		const all = [...staticProtected, ...guards];
		return (rel) => all.some((re) => re.test(rel));
	};

	/**
	 * Enumerate one workspace's buckets, skipping its staging area.
	 *
	 * The staging area is a sibling of the buckets under the store root, so it
	 * would otherwise be walked as if it were a session directory. It only needs
	 * skipping when it sits directly under the store root — an absolute `workDir`
	 * pointing somewhere else is not in the enumeration to begin with.
	 * @param workspace - absolute workspace root.
	 * @returns the bucket descriptors.
	 */
	const bucketsOf = (workspace) => {
		const storeRoot = storeRootFor(workspace, options.storeRoot);
		const staging = workDirFor(storeRoot);
		const stagingRel = staging === null ? null : toPosix(relative(storeRoot, staging));
		const skip = stagingRel !== null && stagingRel.length > 0 && !stagingRel.startsWith("..") && !stagingRel.includes("/")
			? stagingRel
			: undefined;
		return listBuckets(storeRoot, skip);
	};
	 ...
	 * @param root - absolute workspace root.
	 * @param storeRoot - absolute store root (where the buckets actually live).
	 * @param manifest - the just-written manifest.
	 */
	async function review(session, state, root, storeRoot, manifest) {
		...
		for (const item of items) {
			// 暂存区条目必须最先定案,且【绝不能】走盲区还原。
			// 归档步骤是无条件把 work/ 里的东西全部搬走,所以任何"还原"都会在下一轮
			// 被立刻重收 —— 构成可证明的死循环(线上实测:PNG 是二进制,被判盲区,
			// 于是每轮还原一次,连带一次没必要的模型调用)。
			// 而且还原到暂存区在语义上没有意义:那里本来就是它的来处。
			// 人类仍可用 scratch_restore 显式捞回。
			if (item.kind === "workdir") {
				record.kept.push({ path: item.path, reason: "暂存区条目:已按约定归档,自动路径不还原(还原会在下一轮被重收)" });
				record.preResolved.push({ path: item.path, action: "keep", reason: "暂存区约定" });
				continue;
			}
			const info = await previewOf(storedSource(storeRoot, root, item), options.aiReview.previewBytes);
			...
			if (options.aiReview.restoreWhenUninspectable !== false && blindKind(info)) {
				...
				if (Array.isArray(options.__faults) && options.__faults.includes("restore-pre-resolved")) {
					...
					try {
						done = await restoreItem(root, storeRoot, item, "review", forceCopy);
					} catch (error) {
		...
		const userText = [
			"工作区: " + toPosix(root),
			"隔离存储: " + toPosix(storeRoot) + "(在工作区之外;条目 origin 为 store 时相对它寻址)",
		...
		for (const item of pending) {
			...
			if (action === "restore") {
				try {
					if (await restoreItem(root, storeRoot, item, "review", forceCopy)) record.restored.push(item.path);
					else record.problems.push("还原未生效(目标已存在或源缺失): " + item.path);
				} catch (error) {
					record.problems.push("还原失败 " + item.path + ": " + String(error && error.message ? error.message : error));
				}
	 ...
	 * @param isProtectedHere - this sweep's protection predicate.
	 * @returns true when the candidate is eligible to move.
	 */
	function eligible(rel, promptText, isProtectedHere) {
		if (isProtectedHere(rel)) return false;
	...
	async function sweep(session, state) {
		...
		// Where this workspace's buckets and staging area live. Outside the
		// workspace by default — see storeRootFor.
		const storeRoot = storeRootFor(absoluteRoot, options.storeRoot);
		const workAbsolute = workDirFor(storeRoot);
		const workRel = workAbsolute !== null && isInside(absoluteRoot, workAbsolute)
			? toPosix(relative(absoluteRoot, workAbsolute))
			: null;
		const isProtected = protectorFor(workRel);

		const bucket = join(storeRoot, session.id, "turn-" + state.turn);
		const records = [];
		const seen = new Set();

		/**
		 * Queue one candidate, keyed by its absolute path.
		 *
		 * `base` is the root the item is addressed against: implicitly the
		 * workspace, or explicitly the store root for staging leftovers, which now
		 * live outside the workspace and would otherwise fail containment. The
		 * base travels with the record because it decides both where the item
		 * lands inside the bucket and where a restore puts it back.
		 * @param absolute - absolute candidate path.
		 * @param kind - manifest reason tag.
		 * @param base - addressing root; defaults to the workspace.
		 */
		const add = (absolute, kind, base) => {
			const against = base === undefined ? absoluteRoot : base;
			if (records.length >= options.maxQuarantinePerTurn) return;
			if (!isInside(against, absolute)) return;
			if (seen.has(absolute)) return;
			seen.add(absolute);
			records.push({ absolute, kind, base: against, rel: toPosix(relative(against, absolute)) });
		};
		...
		for (const produced of state.produced) {
			...
			if (!existsSync(absolute)) continue;
			const rel = toPosix(relative(absoluteRoot, absolute));
			if (eligible(rel, state.promptText, isProtected)) add(absolute, "created-this-turn");
		}
		...
		for (const name of options.orphanStores) {
			...
			add(absolute, "orphan-store");
		}

		// (4) 工作暂存区:把 agent 本轮停在这里的东西整体归档进桶。
		// 不走 eligible():agent 是主动把它放进暂存区的,不需要再按命名规则猜一遍。
		// 以存储根为基准寻址 —— 暂存区默认在工作区之外,按工作区寻址会直接被
		// 「必须在工作区内」的包含性检查丢掉。硬保护仍由 add() 把关。
		if (workAbsolute !== null) {
			let workEntries = [];
			try {
			...
			for (const name of workEntries) {
				add(join(workAbsolute, name), "workdir", storeRoot);
			}
		}
		...
		const manifest = {
			...
			store: toPosix(storeRoot),
			restoreHint: "按 items[].path 还原:origin 为 workspace(默认)时相对 workspace,为 store 时相对 store。桶内文件位置看 items[].storedAt,它始终相对 store。",
			items: []
		};
		for (const record of records) {
			try {
				const origin = record.base === absoluteRoot ? "workspace" : "store";
				manifest.items.push(await quarantine(storeRoot, record.base, record.absolute, bucket, record.kind, origin, forceCopy));
			} catch (error) {
		...
		await persist();
		log("turn " + state.turn + ": quarantined " + manifest.items.length + " item(s) -> " + toPosix(bucket));
		...
		if (options.aiReview.enabled === false) {
			...
			try {
				manifest.review = await review(session, state, absoluteRoot, storeRoot, manifest);
			} catch (error) {
				manifest.review = {
					at: new Date().toISOString(),
					outcome: "failed",
		...
		ctx.inject(["tools"], (toolCtx) => {
			...
			toolCtx.tools.register(defineTool({
				...
				async execute(args, exec) {
					...
					if (workspace === undefined) return { enabled: false, path: "", relative: "" };
					// The staging area lives under the store root, i.e. outside the
					// workspace, so it has to be resolved from there and not from the
					// workspace the way it used to be.
					const storeRoot = storeRootFor(workspace, options.storeRoot);
					const absolute = workDirFor(storeRoot);
					if (absolute === null) return { enabled: false, path: "", relative: "" };
					try {
					...
					return { enabled: true, path: toPosix(absolute), relative: toPosix(relative(storeRoot, absolute)) };
				}
			}));
			...
			toolCtx.tools.register(defineTool({
				...
				async execute(args, exec) {
					...
					let total = 0;
					let restoredTotal = 0;
					for (const bucket of await bucketsOf(workspace)) {
				...
				async execute(args, exec) {
					...
					for (const bucket of await bucketsOf(workspace)) {
						...
						for (const item of items) {
							...
							try {
								if (await restoreItem(workspace, storeRoot, item, "tool", forceCopy)) {
			...
			toolCtx.tools.register(defineTool({
				...
				async execute(args, exec) {
					...
					for (const bucket of await bucketsOf(workspace)) {
						if (typeof args.session === "string" && args.session.length > 0 && bucket.session !== args.session) continue;
						if (typeof args.turn === "string" && args.turn.length > 0 && bucket.turn !== args.turn) continue;
						const target = toPosix(relative(storeRoot, bucket.dir));
						try {
							await rm(bucket.dir, { recursive: true, force: true });
							await pruneEmptyDirs(dirname(bucket.dir), storeRoot);
							removed.push(target);
						} catch (error) {
	...
	log("loaded (dryRun=" + String(options.dryRun) + ", markTool=" + String(options.markTool !== false) + ", aiReview=" + String(options.aiReview.enabled !== false) + ", store=" + (options.storeRoot === "workspace" ? "in-workspace " + TRASH_DIR : options.storeRoot && String(options.storeRoot).trim() !== "" ? String(options.storeRoot) : "~/" + toPosix(STORE_PARENT) + "/<workspace-key>") + ")");
}

// Exported for the test harness, which derives the store path through the same
// function the plugin uses rather than duplicating the key scheme and silently
// drifting from it.
export { storeRootFor, workspaceKey };
