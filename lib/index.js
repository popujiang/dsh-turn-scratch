...
function workspaceKey(absoluteRoot) {
	...
	// Windows paths are case-insensitive, so normalize before hashing: D:\Share
	// and d:\share are one workspace and must not end up with two stores.
	const hash = createHash("sha1").update(toPosix(absolute).toLowerCase()).digest("hex").slice(0, 8);
	const slug = basename(absolute).replace(/[^\w.-]+/g, "_").replace(/^_+|_+$/g, "").slice(0, 32);
